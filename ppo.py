...
class PPOBuffer:
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buffer, act=self.act_buffer, ret=self.ret_buffer,
                    adv=self.adv_buffer, logp=self.logp_buffer)
        return {k: torch.as_tensor(v, dtype=torch.float32) for k, v in data.items()}

def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
        steps_per_epoch=4000, epochs=7500, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
        ...
        def compute_loss_pi(data):
            ...
            clipped = ratio.lt(1 - clip_ratio) | ratio.rt(1 + clip_ratio)
